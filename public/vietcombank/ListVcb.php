...
$title = 'DANH SÁCH TÀI KHOẢN VIETCOMBANK | ' . $NNL->site('title');
        ...
        <div class="wrapper-box">
            ...
            <div class="content">
                <h2 class="intro-y text-lg font-medium mt-10">
                    Danh sách tài khoản Vietcombank </h2>
                <div class="grid grid-cols-12 gap-6 mt-5">
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                        <button onclick="window.location.href='<?= BASE_URL('vcb/addvcb') ?>'" class="btn btn-primary shadow-md mr-2" type="button">Thêm tài khoản Vietcombank</button>
                    </div>
                    <!-- BEGIN: Data List -->
                    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                        <table class="table table-report -mt-2" id="mytable">
                            ...
                            <tbody>
                                <?php
                                $getData = $NNL->get_list("SELECT * FROM `vietcombank` WHERE `user_id`='" . $getUser['id'] . "'");
                                foreach ($getData as $mbbank) { ?>
                                    <tr class="intro-x">
                                        <td>
                                            <div class="font-medium whitespace-nowrap"><?= $mbbank['username'] ?></div>
                                        </td>
                                        <td class="font-medium whitespace-nowrap"><?= $mbbank['name'] ?></td>
                                        <td class="font-medium whitespace-nowrap"><?= $mbbank['account'] ?></td>
                                        <td class="font-medium whitespace-nowrap"><?= format_cash(getMoney_vcb($mbbank['token'])) ?>đ</td>
                                        ...
                                        <td class="font-medium whitespace-nowrap">
                                            <div class="flex">
                                                ...
                                                href="<?= BASE_URL('vcb/history/') ?><?= $mbbank['account'] ?>"> <i
                                                ...
                                                <a class="flex items-center" style="color:red" href="#" onclick="DeleteVcb(<?= $mbbank['id'] ?>)"> <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Delete </a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php } ?>
