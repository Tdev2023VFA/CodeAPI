...
class MomoV2
{
    ...
    private $URLAction = array(
        ...
        "TRANS_HISTORY_NEW" 					=> "https://api.momo.vn/sync/transhis/get-last-four-weeks",//check lsgd
        "TRANS_HISTORY_CHAT" 					=> "https://m.mservice.io/helios/chat-api/v1/room/get/rooms",//check lsgd
        "TRANS_LOAD_MESSAGE" 					=> "https://m.mservice.io/helios/chat-api/v1/room/load/messages",//check lsgd
    ...
    public function CheckHis($from, $to, $limit)
    {
        $result = $this->TRAN_HIS_LIST($from, $to, $limit);
        if(empty($result)){
            return array(
                "status" => "error",
                "code"   => -5,
        ...
        return json_encode(array(
            "status" => true,
              'message' => 'Thành công',
              "momoMsg" => array("tranList" => $HisList)
    ...
    public function CheckHistoryV3()
    {
        $result = $this->getHistoryChat();
        ...
        //     );
        // }
        $HisList = $result["json"]["data"];
        $tranList = array();
        foreach ($HisList as $transaction){
            $result1 = $this->loadMessageChat($transaction['id']);
                $tranList[] = array(
                    'partnerId'    => $result1['json']['data'][0]['senderId'],
                    'amount'    => json_decode($result1['json']['data'][0]['templateData'],true)['amount'],
                    'comment'    => json_decode($result1['json']['data'][0]['templateData'],true)['message'],
                    'tranId'    => $result1['json']['data'][0]['requestId'],
                    'time'      => date('h:i:s d-m-Y',$transaction['lastMessageAt']/1000)
                );
    ...
    public function CheckHisNew($hours)
    {
    ...
    public function getHistoryChat()
    {
        ...
        $result =  $this->CURL("TRANS_HISTORY_CHAT", $header, $Data);
        return $result;
      
        
    }
    public function loadMessageChat($id)
    {
        $header = array(
            "authorization: Bearer " . $this->config["authorization"],
            "user_phone: " . $this->config["phone"],
        ...
        $result =  $this->CURL("TRANS_LOAD_MESSAGE", $header, $Data);
        return $result;
      
        
    }
    public function getLsgdv2($hours)
    {
        $begin =  (time() - (3600 * $hours)) * 1000;
        //$begin = strtotime("-1 minutes") * 1000;
        $header = array(
        ...
        $result =  $this->CURL("TRANS_HISTORY_NEW", $header, $Data);
       
    }
    public function Checktest($hours)
    {
        $begin =  (time() - (3600 * $hours)) * 1000;
        //$begin = strtotime("-1 minutes") * 1000;
        $header = array(
    ...
    public function CheckHisNewV2($hours)
    {
        $begin =  (time() - (3600 * $hours)) * 1000;
        //$begin = strtotime("-1 minutes") * 1000;
        $header = array(
    ...
    public function CheckHisNew1h($hours)
    {
        $begin =  (time() - (3600 * $hours)) * 1000;
        //$begin = strtotime("-1 minutes") * 1000;
        $header = array(
    ...
    public function TRAN_HIS_LIST_NEW_HOT($from, $to, $limit)
    {
